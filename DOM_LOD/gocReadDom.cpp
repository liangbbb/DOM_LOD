...
static myobj::Material::Map parseTextureMap(const std::string& ss, myobj::Material::Map::TextureMapType type)
{
	myobj::Material::Map map;
	std::string s(ss);
	for (;;)
	{
		if (s[0] != '-')
...
bool  myobj::Model::readline(std::istream& fin, char* line, const int LINE_SIZE)
{
...
std::string  myobj::Model::lastComponent(const char* linep)
{
...
bool  myobj::Model::readMTL(std::istream& fin)
{
...
bool  myobj::Model::readOBJ(std::istream& fin, const osgDB::ReaderWriter::Options* options)
{
	...
	while (fin)
	{
		...
		else if (strlen(line) > 0)
		{
				...
				strncmp(line, "f ", 2) == 0)
			{
				...
				{
					Element::CoordinateCombination coordateCombination = element->getCoordinateCombination();//此处判断一个三角面是否有v n t
...
void  myobj::Model::addElement(Element* element)
{
...
osg::Vec3  myobj::Model::averageNormal(const Element& element) const
{
	osg::Vec3 normal;
...
osg::Vec3  myobj::Model::computeNormal(const Element& element) const
{
	osg::Vec3 normal;
...
bool myobj::Model::needReverse(const Element& element) const
{
